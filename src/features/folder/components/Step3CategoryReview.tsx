...
interface Step3CategoryReviewProps {
...
export function Step3CategoryReview({
  formData,
  onFormDataChange,
  onComplete,
  onSkip
}: Step3CategoryReviewProps) {
  // Get all photos from all categories
  const allPhotos = formData.categories.flatMap(category => 
    ...
    if (trimmedTag && !currentPhoto.tags.includes(trimmedTag)) {
      ...
      onFormDataChange({
        ...
        categories: formData.categories.map(category => {
          if (category.id === currentPhoto.categoryId) {
            return {
              ...category,
              pictures: category.pictures.map(pic =>
